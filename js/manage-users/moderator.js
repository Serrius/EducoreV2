...
(function () {
  ...
  const GROUP = "moderators";
  const STATUSES = ["Active", "Inactive", "Archived"];
  const EMPTY_MSG = {
    "Active": "No active moderators found.",
    "Inactive": "No inactive moderators found.",
    "Archived": "No archived moderators found."
  };

  const UI = {
    Active: {
      tbody: "#moderatorsActiveTbody",
      meta: "#moderatorsActiveMeta",
      pag: "#moderatorsActivePagination",
      bulkBar: "#moderatorsActiveBulkBar",
      selCount: "#moderatorsActiveSelectedCount",
      btnSelectAll: "#moderatorsActiveSelectAllBtn",
      btnClear: "#moderatorsActiveClearSelectionBtn",
      btnExport: "#moderatorsActiveExportSelectedBtn",
      btnArchive: "#moderatorsActiveArchiveSelectedBtn",
    },
    Inactive: {
      tbody: "#moderatorsInactiveTbody",
      meta: "#moderatorsInactiveMeta",
      pag: "#moderatorsInactivePagination",
      bulkBar: "#moderatorsInactiveBulkBar",
      selCount: "#moderatorsInactiveSelectedCount",
      btnSelectAll: "#moderatorsInactiveSelectAllBtn",
      btnClear: "#moderatorsInactiveClearSelectionBtn",
      btnExport: "#moderatorsInactiveExportSelectedBtn",
      btnArchive: "#moderatorsInactiveArchiveSelectedBtn",
    },
    Archived: {
      tbody: "#moderatorsArchivedTbody",
      meta: "#moderatorsArchivedMeta",
      pag: "#moderatorsArchivedPagination",
      bulkBar: "#moderatorsArchivedBulkBar",
      selCount: "#moderatorsArchivedSelectedCount",
      btnSelectAll: "#moderatorsArchivedSelectAllBtn",
      btnClear: "#moderatorsArchivedClearSelectionBtn",
      btnExport: "#moderatorsArchivedExportSelectedBtn",
      btnRestore: "#moderatorsArchivedRestoreSelectedBtn",
    },
  };

  const common = {
    search: "#moderatorsSearch",
    pageSize: "#moderatorsPageSize",
    btnAdd: "#btnAddModeratorTab",
    btnImport: "#btnImportModeratorsCsv",
    exportActiveCsv: "#exportModeratorsActiveCsv",
    exportInactiveCsv: "#exportModeratorsInactiveCsv",
    exportArchivedCsv: "#exportModeratorsArchivedCsv",
    exportAllCsv: "#exportModeratorsAllCsv",
  };
    ...
    try {
      ...
      if (changed) {
        ...
        if (document.visibilityState === "visible") {
          console.log("Moderators table updated");
        }
      }
    } catch (e) {
      console.warn("[moderator] Poll failed:", e?.message || e);
    } finally {
    ...
    const moreMenu = `
      <div class="dropdown d-inline-block">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
          <li>
            <a class="dropdown-item mu-moderator-view-one" href="#" data-id="${sid}">
              <i class="bi bi-eye me-2"></i>View
            </a>
          </li>
          <li>
            <a class="dropdown-item mu-moderator-edit-one" href="#" data-id="${sid}">
              <i class="bi bi-pencil-square me-2"></i>Edit
            </a>
          </li>
          <li><hr class="dropdown-divider"></li>
          <li>
            <a class="dropdown-item mu-moderator-reset-one" href="#" data-id="${sid}">
  ...
  function exportSelected(status) {
    ...
    exportRowsToCsv(`moderators_${status.toLowerCase()}_selected.csv`, rows);
  }
  ...
  async function exportAllStatus(status) {
    ...
    exportRowsToCsv(`moderators_${status.toLowerCase()}.csv`, rows);
  }
  ...
  async function exportAllGroups() {
    ...
    exportRowsToCsv(`moderators_all.csv`, rows);
  }
  ...
  function bindAddStaffOpen() {
    ...
    const modalEl = S.qs("#modalAddModerator");
    if (modalEl) {
  ...
  function bindCreateStaff() {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      const payload = {
        action: "create_staff",
        role,
        id_number: String(S.qs("#moderator_id_number")?.value || "").trim(),
        email: String(S.qs("#moderator_email")?.value || "").trim(),
        first_name: String(S.qs("#moderator_first_name")?.value || "").trim(),
        middle_name: String(S.qs("#moderator_middle_name")?.value || "").trim(),
        last_name: String(S.qs("#moderator_last_name")?.value || "").trim(),
        suffix: String(S.qs("#moderator_suffix")?.value || "").trim(),
        program: String(S.qs("#moderator_program")?.value || "").trim(), // NEW
        status: String(S.qs("#moderator_status")?.value || "Active").trim(),
      };

      try {
        const res = await S.postJSON(payload);

        S.hideModal("modalAddModerator");
  ...
  function bindSaveStaffButton() {
    const btn = S.qs("#btnSaveModerator");
    if (!btn) return;
  ...
  function fillViewStaff(data) {
    ...
    set("#viewModeratorIdNo", String(u?.id_number || "—"));
    set("#viewModeratorEmail", String(u?.email || "—"));
    set("#viewModeratorStatus", String(u?.status || "—"));
    set("#viewModeratorFirstName", String(u?.first_name || "—"));
    set("#viewModeratorMiddleName", String(u?.middle_name || "—"));
    set("#viewModeratorLastName", String(u?.last_name || "—"));
    set("#viewModeratorSuffix", String(u?.suffix || "—"));
    set("#viewModeratorProgram", String(u?.program || "—")); // NEW
    set("#viewModeratorRole", roleLabel(u));
    set("#viewModeratorCreated", fmtDate(u?.created_at || ""));
    set("#viewModeratorLastLogin", fmtDate(u?.last_login_at || ""));
  }
  ...
  function fillEditStaff(data) {
    ...
    setVal("#moderator_edit_user_id", u?.id || "");
    setVal("#moderator_edit_id_number", u?.id_number || "");
    setVal("#moderator_edit_email", u?.email || "");
    setVal("#moderator_edit_first_name", u?.first_name || "");
    setVal("#moderator_edit_middle_name", u?.middle_name || "");
    setVal("#moderator_edit_last_name", u?.last_name || "");
    setVal("#moderator_edit_suffix", u?.suffix || "");
    setVal("#moderator_edit_program", u?.program || ""); // NEW
    setVal("#moderator_edit_role", u?.role || inferDefaultRoleFromActiveTab());
    setVal("#moderator_edit_status", u?.status || "Active");
    setVal("#moderator_edit_created_at", u?.created_at || "");
    
    // Populate program dropdown with user's program selected
    setTimeout(() => {
      populateStaffProgramDropdown("#moderator_edit_program", u?.program || "");
    }, 50);
  }

  function bindEditStaffSubmit() {
    const form = S.qs("#formEditModerator");
    if (!form) return;
    ...
    form.addEventListener("submit", async (e) => {
      ...
      const payload = {
        action: "update_staff",
        id: Number(S.qs("#moderator_edit_user_id")?.value || 0),
        role: String(S.qs("#moderator_edit_role")?.value || "").trim(),
        id_number: String(S.qs("#moderator_edit_id_number")?.value || "").trim(),
        email: String(S.qs("#moderator_edit_email")?.value || "").trim(),
        first_name: String(S.qs("#moderator_edit_first_name")?.value || "").trim(),
        middle_name: String(S.qs("#moderator_edit_middle_name")?.value || "").trim(),
        last_name: String(S.qs("#moderator_edit_last_name")?.value || "").trim(),
        suffix: String(S.qs("#moderator_edit_suffix")?.value || "").trim(),
        program: String(S.qs("#moderator_edit_program")?.value || "").trim(), // NEW
        status: String(S.qs("#moderator_edit_status")?.value || "Active").trim(),
      };

      try {
        await S.postJSON(payload);
        S.hideModal("modalEditModerator");
        await refresh();
    ...
    const updateBtn = S.qs("#btnUpdateModerator");
    if (updateBtn) {
  ...
  function bindRowActions() {
    ...
    document.addEventListener("click", async (e) => {
      const a = e.target.closest(".mu-moderator-view-one, .mu-moderator-edit-one, .mu-moderator-reset-one");
      ...
      if (a) {
        ...
        try {
          if (a.classList.contains("mu-moderator-view-one")) {
            const data = await loadUser(id);
            fillViewStaff(data);
            S.showModal("modalViewModerator");
          } else if (a.classList.contains("mu-moderator-edit-one")) {
            const data = await loadUser(id);
            fillEditStaff(data);
            S.showModal("modalEditModerator");
          } else if (a.classList.contains("mu-moderator-reset-one")) {
            const doIt = async () => {
