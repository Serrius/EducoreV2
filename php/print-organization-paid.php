<?php
// php/print-organization-paid.php
// Print ALL PAID organization fee records (org_id + term_id optional)
// - Data: organization_fee_payments + users + organizations + academic_terms
// - Signatures: Treasurer + Auditor (YEAR-based Option B) + ACTIVE e_signatures
//
// Fixes:
// - term_id optional: picks latest term that HAS payments for this org; fallback Active term; fallback latest term
// - Permission updated to YEAR-based officers (Option B) OR admin roles
// - Officer lookup updated with YEAR-based fallback + users name fallback
...
function session_role(): string {
  ...
  if (isset($_SESSION['user']) && is_array($_SESSION['user']) && isset($_SESSION['user']['role'])) return (string)$_SESSION['user']['role'];
  return '';
}
function img_tag_base64(string $relativePath, string $className = '', string $alt = ''): string {
  ...
  // If stored as URL, skip (we embed from filesystem)
...
/* =========================
   ...
   ========================= */
if ($termId <= 0) {
  // 1) latest term that actually has payments for this org
  $stLastPayTerm = $pdo->prepare("
    SELECT p.academic_term_id
    FROM organization_fee_payments p
    WHERE p.org_id = :org_id
    ORDER BY p.paid_at DESC, p.id DESC
    LIMIT 1
  ");
  $stLastPayTerm->execute([':org_id' => $orgId]);
  $termId = (int)($stLastPayTerm->fetchColumn() ?: 0);
...
/* =========================
   Permission
   - allow admins OR active officer in same school_year (Option B)
   ========================= */
$role = session_role();
$isAdminRole = in_array($role, ['overseer','super_admin','special_admin','faculty_admin','moderator'], true);

if (!$isAdminRole && !is_officer_for_year($pdo, $viewerId, $orgId, $termId)) {
...
/* =========================
   ...
   ========================= */
$stOrg = $pdo->prepare("
  SELECT id, org_type, org_name, abbreviation, logo_path, scope, program_id, fee_required, status
  FROM organizations
  WHERE id = :id
  LIMIT 1
");
...
/* =========================
   Fetch PAID list (organization fees)
   ========================= */
$stRows = $pdo->prepare("
  SELECT
    u.id_number,
...
$tableHtml = '';
if (count($rows) <= 0) {
  $tableHtml = '<tr><td colspan="5" class="muted" style="text-align:center; padding:14px;">No paid students found.</td></tr>';
} else {
  $i = 0;
  foreach ($rows as $r) {
    $i++;
...
<head>
  <meta charset="utf-8">
  <title>Organization Fee Paid List</title>
  <style>
    @page { margin: 8mm; }
    ...
    .td-amt { width: 120px; text-align: right; font-weight: 700; }
  ...
  <div class="wrap">
    <table class="header">
      <tr>
          ...
          <table class="orgbox">
            <tr>
              ...
              <td style="padding-left:6px;">
                ...
                <div class="orgmeta">
                  {$orgTypeEsc} {$orgAbbrEsc} • {$scopeEsc}<br/>
                  {$termTextEsc}
                </div>
              </td>
            </tr>
        ...
        <td class="h-right">
          <div class="title">PAID STUDENTS LIST</div>
          <div class="meta">
...
try {
  ...
  $dlName = 'OrgFeePaid_' . preg_replace('/[^0-9A-Za-z_-]+/', '_', (string)$orgName) . "_T{$termId}.pdf";
