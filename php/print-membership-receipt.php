<?php
// php/print-membership-receipt.php
// Membership Receipt (PDF)
// - Receipt source: organization_membership_receipts
// - Membership source: organization_memberships
// - Org source: organizations
// - Term source: academic_terms
// - Student source: users
// - Signatures: Treasurer + Auditor officers of the org (organization_officers) + their ACTIVE e-signatures (e_signatures)
//
// FIXES:
...
/**
 * Convert a stored relative path like "assets/uploads/..." into an mPDF-friendly base64 image tag.
 */
...
if ($receiptId <= 0 && $receiptNoParam === '') {
  ...
  header('Content-Type: text/plain; charset=utf-8');
  echo 'Missing receipt_id / id / receipt_no.';
  exit;
}

/* =========================
   Fetch receipt + membership + org + term + student
   ========================= */
$sql = "
  SELECT
    r.id AS receipt_id,
...
$statusRaw = (string)($row['membership_status'] ?? 'Approved');
$statusLabel = $statusRaw !== '' ? $statusRaw : 'Approved';

$schoolYear = (string)($row['school_year'] ?? '');
$semester   = (string)($row['semester'] ?? '');
$termText   = term_label($schoolYear, $semester);
...
$orgLogoHtml = '';
if ($orgLogoPath !== '') {
  $orgLogoHtml = img_tag_base64($orgLogoPath, 'org-logo', 'Logo');
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .receipt-wrapper { width: 100%; margin: 0; border: 1px solid #ccc; padding: 12px 16px; }
...
<body>
  <div class="receipt-wrapper">
    ...
    <table class="meta-table">
      ...
      <tr><td class="meta-label">Receipt For:</td><td>Organization / Club Membership Fee</td></tr>
      <tr><td class="meta-label">Academic Term:</td><td>{$termTextEsc}</td></tr>
      <tr><td class="meta-label">Date Paid:</td><td>{$paidOnTextEsc}</td></tr>
      <tr><td class="meta-label">Status:</td><td>{$statusLabelEsc}</td></tr>
    </table>
...
try {
  ...
  $downloadName = 'MembershipReceipt_' . preg_replace('/[^0-9A-Za-z_-]+/', '_', $receiptNo) . '.pdf';
